...
mkdir nb_analysis_static/lig_interactions -p

nb_analysis_dir="$(pwd)/nb_analysis_static"
...
for ((i=$start_res; i<=$end_res-1; i++)); do
  for ((j=$i+1; j<=$end_res; j++)); do
    pairwise_outfile="nb_analysis_static/.pairwise-${i}-${j}.txt"
    avg_outfile="nb_analysis_static/avg-${i}-${j}.dat"
    vmap_outfile="nb_analysis_static/.map.elec.${i}-${j}.gnu"
    emap_outfile="nb_analysis_static/.map.vdw.${i}-${j}.gnu"
...
python3 /home/dexter/Desktop/lauren/MASTER_amber/transformers/nb_analysis-static.py $nb_analysis_dir

echo "Python script execution complete."
wait

cd nb_analysis_static/
